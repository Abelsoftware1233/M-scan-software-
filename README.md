...
## Productie-deployment (systemd)

Voor een echte server (i.p.v. de losse `python app.py` dev-server) is er
`deploy.sh` — dit zet de app op als systemd-service, draaiend onder
`gunicorn` in een eigen venv, met een dedicated unprivileged systeemgebruiker.
...
Dit script:
- maakt een systeemgebruiker `malscan` aan (geen login/shell)
- kopieert de code naar `/opt/malscan`
- zet een venv op en installeert dependencies + gunicorn
- schrijft en start een systemd-unit `malscan.service`
- bindt alleen op `127.0.0.1:5777` (niet extern bereikbaar)

**Nginx/TLS wordt bewust niet door dit script geregeld** — richt je eigen
reverse proxy in met `proxy_pass http://127.0.0.1:5777;`.

Beheer na deployment:
```bash
systemctl status malscan
systemctl restart malscan     # na een code-update
journalctl -u malscan -f      # live logs
```
...
├── deploy.sh                    ← systemd-deployment script (venv + gunicorn)
├── requirements.txt
├── LICENSE
├── .gitignore
├── templates/
